...
BASE_URL = 'http://www.typhoon2000.ph/multi/log.php'
...
def proc_tc_data(tc_name, year, time_index, base_url=BASE_URL, dload_url=None, exclude=[], timestamp=pd.to_datetime(pd.datetime.now())):
    ...
    if (r.status_code == 200):
        ...
        res = re.search('preload(.*)play\(\);', res).group(1).strip();
        res = [s for s in re.split('[a-z]+\[[0-9]+\] = ', res) if re.search('={3,}', s)]

        dfs = []
        for s in res:
            s = re.sub('\\\\n', ' ', s)
            update_time = pd.to_datetime(re.search('\((.*UTC)\)', s).group(1))

            s1 = re.search('=+(.*)', s).group(1).strip().split(':')
            centers = [re.search('[A-Z]{3,}', s2).group(0) for s2 in s1 if re.search('[A-Z]{3,}', s2)]
            info = [re.search('.*KT', s2).group(0).strip() for s2 in s1 if re.search('.*KT', s2)]
            ...
            dfs.append(out_df[['Center', 'Date', 'Lat', 'Lon', 'PosType', 'Vmax', 'Cat']])
        if time_index == -1:
            time_index = len(dfs) - 1
        return dfs[time_index]
    return None
...
if __name__ == '__main__':
    ...
    parser.add_argument('tc_year', help='TC year')
    parser.add_argument('output', help='Output CSV')
    parser.add_argument('--time-index', help='TC year', default=-1)
    ...
    df = proc_tc_data(args.tc_name.upper(), args.tc_year, time_index=args.time_index)
